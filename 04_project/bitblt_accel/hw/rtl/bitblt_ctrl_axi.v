...
// The temporary eight-cycle engine proves register, status and IRQ behavior.
module bitblt_ctrl_axi #(
    ...
    reg busy_reg, done_reg, error_reg, interrupt_reg;
    reg [3:0] work_count;
    ...
    function [31:0] register_read;
        ...
        begin
            case (word_address)
                ...
                6'h01: register_read = {29'h0, error_reg, done_reg, busy_reg};
    ...
    always @(posedge axi_aclk or negedge axi_resetn) begin
        if (!axi_resetn) begin
            ...
            busy_reg <= 0; done_reg <= 0; error_reg <= 0;
            work_count <= 0; interrupt_reg <= 0;
        end else begin
            interrupt_reg <= 0;
                    ...
                    case (awaddr_reg[7:2])
                        6'h00: begin
                            ...
                            if (axi_wdata[0]) begin
                                if (!busy_reg) begin
                                    busy_reg <= 1; done_reg <= 0; work_count <= 8;
            ...
            if (busy_reg) begin
                if (work_count > 1) work_count <= work_count - 1'b1;
                else begin
                    work_count <= 0; busy_reg <= 0; done_reg <= 1;
                    interrupt_reg <= 1;
                end
            end
        end
