...
- 日期：2026-09-19
- 提出：C（RISC-V 软件）
- 状态：**待 A/B 确认**。本文件是草案，不是已冻结基线。
- 待确认项一律标注 `待A确认` / `待B确认`，C 侧在确认前不会写进代码。

依据：

- `rwj/udp_image_demo/fpga/rtl/top.v` 实测信号（A 组现有实现）
...
> 但文件名里的 `_draft` **必须保留到 A/B 确认并冻结数值为止**——
...
```text
APB Slave 0  base = 0xF8100000        （soc.h: IO_APB_SLAVE_0_INPUT）

0xF8100000 - 0xF81000FF   Reserved
0xF8100100 - 0xF81001FF   RX Frame Status   ← 本文件描述的范围
```
...
| 偏移 | 名称 | R/W | 复位值 | 分类 | 更新时机 |
|---:|---|---|---:|---|---|
| `+0x00` | `SEQ` | RO | `0` | **commit 标志** | 快照锁存完成后 +1 |
| `+0x04` | `FRAME_STATUS` | RO | `0` | **snapshot** | 与 SEQ 同拍锁存 |
| `+0x08` | `FRAME_ID` | RO | `0` | **snapshot** | 同上 |
| `+0x0C` | `SLOT` | RO | `0` | **snapshot** | 同上 |
| `+0x10` | `BASE_ADDR` | RO | `0` | **snapshot** | 同上 |
| `+0x14` | `RX_BYTES` | RO | `0` | **snapshot** | 同上 |
| `+0x18` | `EXPECT_BYTES` | RO | `0` | **snapshot** | 同上 |
| `+0x1C` | `ACK_SEQ` | WO | `0` | 控制 | 软件写；硬件不回读 |
| `+0x20` | `LIVE_STATUS` | RO | — | **live** | 实时，不属于任何 SEQ |
| `+0x24` | `ERR_STICKY` | RO | `0` | **live** | 跨帧粘滞，matching ACK 清 |
| `+0x28` | `VERSION` | RO | 待定 | **static** | 常量 |
| `+0x2C–0xFC` | Reserved | RO | `0` | — | 读回 0，写入忽略 |
...
**与现有 RTL 信号的不匹配（需要 A 决策）：**

| # | 问题 | 现状 | 影响 |
|---|---|---|---|
| 1 | **`BASE_ADDR` 在 RTL 里不存在** | 只有 `frame_wslot`(3-bit) 和 `slot_base()` 函数 | 必须**新增锁存寄存器**。不能由软件从 SLOT 推导——迁移文档要求"硬件报回来的实际写入基址必须等于软件授权的那块"，推导值证明不了这件事 |
...
**没有发明 bit。** 依据是 A 侧 RTL 里**已经存在**的一版状态字节——`top.v:721`
组装 UDP 回执包时用了它：
...
RTL 里已存在的 `frame_ack` 是 **UDP 网络层回执包**（`udp_ack_tx` 发出的 24 字节
载荷，见 `top.v:700-727`），与本寄存器毫无关系。命名必须区分开。
...
编码：沿用团队规则——**高 16 位主版本、低 16 位次版本**。

**不冻结具体数值。**

> `待B确认`：不要与已冻结的两个撞车——
> `BITBLT_VERSION_RGB565 = 0x00020000`、`DISPLAY_VERSION_RGB565 = 0x00030000`。
...
1. **`BASE_ADDR` 是否由硬件提供锁存值？**（必须，不能软件推导）
2. **`SLOT` 在联合工程双缓冲下的定义**：收敛成 1 bit，还是保留 3 bit 恒 0/1？
3. **`FRAME_ID` 高 16 位**是否恒 0？（现状 `frame_id_cur` 是 16-bit）
4. **`FRAME_STATUS` 最终 bit 表**：保留原编号（bit1/bit2 留空）还是紧凑重排？
5. **`LEN_ERR` / `NO_DATA_ERR` 是否单列**为 bit6 / bit7？
6. **`ARP_MISS` 移出 FRAME_STATUS、归入 ERR_STICKY** —— 同意否？
   （RTL 证据：它不参与 `frame_ok`）
7. **`CAL_DONE` 从 ACK 包状态字节移出、改放 LIVE_STATUS** —— 同意否？这是行为迁移。
8. **APB slave 在合并后落在哪个时钟域？**（决定 K 节是否真的零 CDC）
9. **ACK 与 publish 同周期的优先级**实现是否如 G.1（publish 优先 + ACK 只清不置）？
   以及 G.3 的推论是否成立？
10. **`0xF8100000` 哑模块替换方案**：如何扩展而不破坏厂商地址译码？
11. **`VERSION` 是否需要**？需要的话数值由谁分配？

# 需要 B 回复的（4 项）

1. **权威头文件的归属**：新增的 `udp_frame_status_regs.h` 放
   `04_project/bitblt_accel/sw/driver/`（与 `bitblt_regs.h` 同级）还是别处？
2. **`VERSION` 号段分配**：不要与 `0x00020000`(BitBlt) / `0x00030000`(Display) 撞车。
3. **C 侧隔离约定是否扩展到第三个头文件**——即 `make check-hw-isolation` 是否加一条
   "只有 `udp_frame_status.c` 可 include 该头文件"。
4. **`FRAME_SWAP_ERR_NOT_READY` 语义超载**如何处理：新增专用错误码，还是继续复用。
...
| 文件 | 性质 |
|---|---|
| `driver/udp_frame_status_regs.h` | 新增，offset 唯一定义点 |
