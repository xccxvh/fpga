...
不得跨4 KiB；通过2048像素的异步FIFO跨到像素时钟域。两级读仲裁器在Burst
边界round-robin；已接受的Burst保持所有权到`RLAST`响应结束。固定显示最高
优先级会在连续扫描时饿死CPU/BitBlt读通道，因此不再作为接口约定。
...
在持续1080p扫描下，CPU/BitBlt的1080p Fill实测为37.39/657.99 MiB/s，
...
Framebuffer地址不能直接互换。团队需先决定最终统一格式、分辨率、DDR地址
及UDP写入路径；若保留RGB565网络传输，需明确在PC或FPGA/CPU端转换为
XRGB8888后再交给当前BitBlt，或共同设计并验证RGB565版BitBlt/显示链路。
在该选择确认前，本V0.4仅表示B组路径已验证，不代表UDP与BitBlt已联合运行。
