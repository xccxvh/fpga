...
命令流程：配置全部参数并执行 `fence rw,rw`，确认 BUSY=0，写 START；硬件原子锁存
参数。完成时 BUSY=0、DONE=1；任何失败同时 ERROR=1。新 START 或 CLEAR 清旧 DONE/ERROR。
...
实际停在哪里。CPU 当前无数据缓存时仍需 `fence rw,rw`；未来启用 D-cache 后，CPU 与
DMA/BitBlt 交接前必须执行相应 clean/invalidate。
...
5. 运行至少 3600 帧，并记录 Efinity 时序、VERSION、串口状态和屏幕现象。
