...
(
	input			in_pclk,
	input			in_rstn,
	input	[PW-2:0]H_SyncPulse,           
	input	[PW-2:0]H_BackPorch,  	          
	input	[PW-1:0]H_ActivePix,	           
	input	[PW:0]	H_FrontPorch,
	input	[PW-2:0]V_SyncPulse,
	input	[PW-2:0]V_BackPorch,
	input	[PW-1:0]V_ActivePix,
	input	[PW-2:0]V_FrontPorch,
	input			in_en,
	
	output 	[PW-1:0]out_dbg_y,
	output 	[PW-1:0]out_x,
	output 	[PW-1:0]out_y,
	output 			out_valid,
	output 			out_de,
...
wire	[PW-1:0]FramePeriod	= V_SyncPulse + V_BackPorch + V_ActivePix +	V_FrontPorch;
wire	[PW-1:0]Vde_Start	= V_SyncPulse + V_BackPorch;
wire	[PW-1:0]Vde_End		= V_SyncPulse + V_BackPorch + V_ActivePix;

/* VGA counter for the output display sync signals generator */
reg [PW+1:0]r_x_cnt;
reg [PW-1:0]r_y_cnt;
reg [PW-1:0]r_x_active_1P;
reg [PW-1:0]r_y_active_1P;
reg [PW-1:0]r_x_active_2P;
reg [PW-1:0]r_y_active_2P;
reg [2:0] 	r_p_cnt;
reg			r_de_vs;
reg			r_vs;
reg			r_de;
...
begin
	...
	begin    
		...
		r_y_active_1P	<= {PW{1'b0}};
		r_x_active_2P	<= {PW{1'b0}};
		r_y_active_2P	<= {PW{1'b0}};
//		r_p_cnt			<= 3'd1;
		r_p_cnt			<= 3'd0;
		r_de			<= 1'b0;
		r_hs			<= 1'b1;
