...
wire frame_stop = {frame_period_r1,frame_period_r0} == 2'b10;
    
assign tx_fifo_wrdata = rx_data[143:120];
always @( posedge clk or negedge rst_n )
begin
    if( !rst_n ) begin
        ...
        5'd3 : begin
            if( ~wr_fifo_full ) begin
                rx_data[143:16] <= rd_fifo_rddata;//data 1
                tx_fifo_valid <= 1'b1;
                pre_state <= pre_state + 1'b1;
            end
        end
        ...
        5'd5 :begin
            if( ~wr_fifo_full ) begin
                ...
                rx_data <= rx_data << 24  ;//data 3
                pre_state <= pre_state + 1'b1 ;
            end 
        end
          
        5'd6 : begin
            if(rd_frame_end && ~wr_fifo_full) begin
                rx_data <= rx_data << 24  ;
                tx_fifo_valid <= 1'b1;
                pre_state <= pre_state + 1'b1;
            end else if( ~rd_fifo_rdempty && ~wr_fifo_full ) begin
                rd_fifo_rden <= 1'b1;
                tx_fifo_valid <= 1'b1;
                rx_data <=  rx_data << 24  ; //data 4
                pre_state <= pre_state + 1'b1 ;
            end
        end
        ...
        5'd8 :begin
            ...
            if( ~wr_fifo_full ) begin
                tx_fifo_valid <= 1'b1;
                rx_data[135:8] <= rd_fifo_rddata;//data 1
                rx_data[143:136] <= rx_data[119:112];
                pre_state <= pre_state + 1'b1 ;
            end 
        end
        ...
        5'd13:begin
            if( ~wr_fifo_full ) begin
            rx_data <= rx_data << 24  ;
            rx_data[127:0] <= rd_fifo_rddata;//data 1
            tx_fifo_valid <= 1'b1;
            pre_state <= pre_state + 1'b1 ;
            end 
        end
        ...
        5'd14:begin
            ...
            // rx_data[143:128] <= rx_data[127:112];
        end
