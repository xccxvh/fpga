...
begin
    if( !rst_n ) begin
        ...
        5'd1 : if( de | frame_cnt_end  ) begin
            pre_state <= pre_state + 1'b1;
            par_pix_data[127:112] <= vin; //data 1
        end
        5'd2 : begin
            if( de | frame_cnt_end ) begin
                pre_state <= pre_state + 1'b1;
                par_pix_data[111:96] <= vin; //data 2
            end 
        end
        5'd3 : begin
            if( de | frame_cnt_end ) begin
                par_pix_data[95:80] <= vin; //data3
                pre_state <= pre_state + 1'b1;
            end
        end
        5'd4 : begin
            if( de | frame_cnt_end ) begin
                par_pix_data[79:64] <= vin; //data 4
                pre_state <= pre_state + 1'b1;
            end
        end
        5'd5 :begin
            if( de | frame_cnt_end ) begin
                par_pix_data[63:48] <= vin;// data 5
                pre_state <= pre_state + 1'b1;
            end
        end
 ...
 assign    fifo_wr_data = par_pix_data;
