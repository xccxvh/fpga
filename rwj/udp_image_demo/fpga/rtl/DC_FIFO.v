...
`timescale 1ns/100ps
  ...
  //2023-1013: V3.7 By Bruce Chen
  //1、修改了格雷码地址计数跨时钟域亚稳态同步器打2拍，增加同步器综合属性注释
  //2、增加了复位信号同步器属性注释
  //////////////////////////////////////////////////////////////////////////////////////
...
//********************************************************/ 
  /////////////////////////////////////////////////////////
 (* async_reg = "true" *) reg   [1:0]         WrClkRstGen   = 2'h3;
 (* async_reg = "true" *) reg   [1:0]         RdClkRstGen   = 2'h3;
...
/*   reg   [AW_C:0]      RdAddrOut   = {AW_C+1{1'h0}}; 
  ...
  (* async_reg = "true" *) reg   [AW_C:0]      RdAddrOut   = {AW_C+1{1'h0}}; 
  (* async_reg = "true" *) reg   [AW_C:0]      WrRdAddr_meta    = {AW_C+1{1'h0}}; 
  (* async_reg = "true" *) reg   [AW_C:0]      WrRdAddr    = {AW_C+1{1'h0}}; 
  
  always @( posedge   WrClk)   
  begin
    if (WrClkRst) begin
        WrRdAddr_meta <= # TCo_C {AW_C+1{1'h0}} ;
        WrRdAddr <= # TCo_C {AW_C+1{1'h0}}     ;
    end
    else  begin
        WrRdAddr_meta <= # TCo_C RdAddrOut [AW_C:0] ;
        WrRdAddr <= # TCo_C WrRdAddr_meta ;
    end
  end
...
/*   reg   [AW_C :0]     RdWrAddr  = {AW_C+1{1'h0}}; 
  ...
  (* async_reg = "true" *)  reg   [AW_C :0]     RdWrAddr  = {AW_C+1{1'h0}}; 
  (* async_reg = "true" *)  reg   [AW_C :0]     RdWrAddr_meta  = {AW_C+1{1'h0}};
  
  always @( posedge   RdClk)    
  begin
    if (RdClkRst )  begin
        RdWrAddr_meta <= # TCo_C {AW_C+1{1'h0}}   ;
        RdWrAddr  <= # TCo_C {AW_C+1{1'h0}}   ;
    end
    else begin
            RdWrAddr_meta  <= # TCo_C FifoWrAddr [AW_C:0]  ; 
            RdWrAddr  <= # TCo_C RdWrAddr_meta  ; 
    end
  end  
...
//********************************************************/ 
  ...
  /////////////////////////////////////////////////////////
  (* async_reg = "true" *)reg  [CW_C:0]  AddrOut;    //(O)Address Output
