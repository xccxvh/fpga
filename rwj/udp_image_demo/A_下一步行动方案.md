...
> ## ⚠️ 本文的合并方向已作废（2026-09-19）
>
> **§9「最省力的路径：不要新建工程，直接在 `udp_image_demo` 上长，把 SoC 当成一个
> 新模块加进去」—— 这条建议作废。**
>
> 2026-09-19 复核发现，B 已经在厂家 `ddr_demo_ti60` 工程上完成了
> SoC + DDR3 三路仲裁 + 1920×1080@60 HDMI + BitBlt + 显示控制器，
> 并已上板验证（含 iverilog 回归、3600 帧老化零欠流）：
> `04_project/bitblt_accel/hw/efinity/overlay/rtl/ddr3_example_top.v:553` 例化 SoC。
>
> 在 `udp_image_demo` 上从头加 SoC 是**重复劳动**，且要重做 B 已解决的
> DDR 仲裁、CDC 和时序收敛。
>
> **正确的合并方向**：把 A 的 UDP 收图通路（`fpga/rtl/ge/`，35 个 .v，
> ~8300 行）搬进 B 的工程，而不是反过来。
>
> **因此以下章节作废：** §3（DDR3 配置差异，已由 A0-2 定案但落在一个可能退役的
> 工程上）、§4 阶段 0/1（A1-1～A1-5，B 已覆盖）、§5 决策 1/3、§6.3 内存布局草案、
> §9 路线图。
>
> **仍然有效：** §2.4 引脚冲突分析、§6.1/6.2 接口总表与寄存器草案（供 B 参考）、
> §7 踩坑清单（SDC 静默丢弃、`build.sh` 被旧 bit 骗、按键引脚复用、DC_FIFO 假负 slack、
> 引脚命名两套）—— 这几条合并时会再次遇到。
>
> **后续请看** `07_docs/interfaces/udp_frame_status_interface_review_v0.1.md`
