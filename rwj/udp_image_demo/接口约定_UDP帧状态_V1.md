...
> ## ⚠️ 本文档已作废（2026-09-19）
>
> 本文是 A 组在**未与 B 组对齐**的情况下独立拟的草案，**从未发出**。
> 2026-09-19 复核 B 的 `07_docs/interfaces/bitblt_interface_v0.2.md`（V0.3）
> 后发现多处实质冲突，且 V0.3 已完成板测：
>
> | 项目 | 本文（作废） | V0.3（以此为准） |
> |---|---|---|
> | 分辨率 | 1280×720 | **1920×1080** |
> | 像素格式 | RGB565 | **XRGB8888** |
> | Framebuffer | 4 槽 × 2 MiB @ `0x0020_0000` | **FB_A/B @ `0x0100_0000`/`0x0180_0000`** |
> | 换帧 | `frame_done` 后自动切 | **NEXT_ADDR → SWAP_REQUEST → VBlank** |
> | 控制总线 | APB slave 0 | **SYSTEM_AXI_A** |
>
> **后续请看** `07_docs/interfaces/udp_frame_status_interface_review_v0.1.md`
>
> 本文中仍然有效的部分：§6.2 的两个已修 bug（BRESP 从未被检查、ACK 回执截断）
> 及 §4 的 BRESP 判据实现。§3 的寄存器表、§5 的软件用法、§7 的地址规划已作废。
